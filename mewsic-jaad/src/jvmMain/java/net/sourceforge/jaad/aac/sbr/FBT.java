...
class FBT implements Constants {
    ...
    public static int qmf_start_channel(int bs_start_freq, int bs_samplerate_mode,
        ...
        int startMin = startMinTable[sample_rate.index];
        int offsetIndex = offsetIndexTable[sample_rate.index];
    ...
    public static int qmf_stop_channel(int bs_stop_freq, SampleFrequency sample_rate,
        ...
        if (bs_stop_freq == 15) {
            ...
            int stopMin = stopMinTable[sample_rate.index];

            /* bs_stop_freq <= 13 */
            return Math.min(64, stopMin + STOP_OFFSET_TABLE[sample_rate.index][Math.min(bs_stop_freq, 13)]);
        }
    }
