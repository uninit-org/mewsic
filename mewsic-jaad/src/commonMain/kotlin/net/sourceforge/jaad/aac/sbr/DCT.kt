...
internal object DCT {
    ...
    private val w_array_real = floatArrayOf(
    ...
    private val w_array_imag = floatArrayOf(
    ...
    private val dct4_64_tab = floatArrayOf(
    ...
    private val bit_rev_tab = intArrayOf(
        0,
        16,
        8,
        24,
    ...
    private fun fft_dif(Real: FloatArray, Imag: FloatArray) {
        var w_real: Float
        var w_imag: Float // For faster access
        var point1_real: Float
        var point1_imag: Float
        var point2_real: Float
        var point2_imag: Float // For faster access
        var j: Int
        var i: Int
        var i2: Int
        var w_index: Int // Counters
        ...
        while (i < 16) {
            ...
            point2_real = Real[i2]
            point2_imag = Imag[i2]
            w_real = net.sourceforge.jaad.aac.sbr.DCT.w_array_real.get(i)
            w_imag = net.sourceforge.jaad.aac.sbr.DCT.w_array_imag.get(i)
        ...
        w_index = 0
        while (j < 8) {
            w_real = net.sourceforge.jaad.aac.sbr.DCT.w_array_real.get(w_index)
            w_imag = net.sourceforge.jaad.aac.sbr.DCT.w_array_imag.get(w_index)
            i = j
            point1_real = Real[i]
            point1_imag = Imag[i]
            i2 = i + 8
        ...
        i = 0
        while (i < net.sourceforge.jaad.aac.sbr.DCT.n) {
            i2 = i + 4
            point1_real = Real[i]
            point1_imag = Imag[i]
            point2_real = Real[i2]
        ...
        w_real = net.sourceforge.jaad.aac.sbr.DCT.w_array_real.get(4) // = sqrt(2)/2
        ...
        w_real = net.sourceforge.jaad.aac.sbr.DCT.w_array_real.get(12) // = -sqrt(2)/2
    ...
    /* size 64 only! */
    fun dct4_kernel(in_real: FloatArray, in_imag: FloatArray, out_real: FloatArray, out_imag: FloatArray) {
        ...
        while (i < 32) {
            ...
            x_re = in_real[i]
            x_im = in_imag[i]
            tmp = (x_re + x_im) * net.sourceforge.jaad.aac.sbr.DCT.dct4_64_tab.get(i)
            in_real[i] = x_im * net.sourceforge.jaad.aac.sbr.DCT.dct4_64_tab.get(i + 64) + tmp
            in_imag[i] = x_re * net.sourceforge.jaad.aac.sbr.DCT.dct4_64_tab.get(i + 32) + tmp
            i++
        }

        /* Step 3: FFT, but with output in bit reverse order */net.sourceforge.jaad.aac.sbr.DCT.fft_dif(
            in_real,
            in_imag
        )
        ...
        while (i < 16) {
            ...
            var x_im: Float
            var tmp: Float
            i_rev = net.sourceforge.jaad.aac.sbr.DCT.bit_rev_tab.get(i)
            x_re = in_real[i_rev]
            x_im = in_imag[i_rev]
            tmp = (x_re + x_im) * net.sourceforge.jaad.aac.sbr.DCT.dct4_64_tab.get(i + 3 * 32)
            out_real[i] = x_im * net.sourceforge.jaad.aac.sbr.DCT.dct4_64_tab.get(i + 5 * 32) + tmp
            out_imag[i] = x_re * net.sourceforge.jaad.aac.sbr.DCT.dct4_64_tab.get(i + 4 * 32) + tmp
            i++
        }
        // i = 16, i_rev = 1 = rev(16);
        out_imag[16] = (in_imag[1] - in_real[1]) * net.sourceforge.jaad.aac.sbr.DCT.dct4_64_tab.get(16 + 3 * 32)
        out_real[16] = (in_real[1] + in_imag[1]) * net.sourceforge.jaad.aac.sbr.DCT.dct4_64_tab.get(16 + 3 * 32)
        i = 17
        while (i < 32) {
            var x_re: Float
            var x_im: Float
