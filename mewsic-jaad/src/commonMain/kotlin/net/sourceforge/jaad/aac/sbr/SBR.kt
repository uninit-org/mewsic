...
class SBR(
    ...
    var qmfa: Array<net.sourceforge.jaad.aac.sbr.AnalysisFilterbank?> =
        arrayOfNulls<net.sourceforge.jaad.aac.sbr.AnalysisFilterbank>(2)
    var qmfs: Array<net.sourceforge.jaad.aac.sbr.SynthesisFilterbank?> =
        arrayOfNulls<net.sourceforge.jaad.aac.sbr.SynthesisFilterbank>(2)
    ...
    init {
        ...
        if (stereo) {
            /* stereo */
            var j: Int
            qmfa[0] = net.sourceforge.jaad.aac.sbr.AnalysisFilterbank(32)
            qmfa[1] = net.sourceforge.jaad.aac.sbr.AnalysisFilterbank(32)
            qmfs[0] = net.sourceforge.jaad.aac.sbr.SynthesisFilterbank(if (downSampledSBR) 32 else 64)
            qmfs[1] = net.sourceforge.jaad.aac.sbr.SynthesisFilterbank(if (downSampledSBR) 32 else 64)
        } else {
            /* mono */
            qmfa[0] = net.sourceforge.jaad.aac.sbr.AnalysisFilterbank(32)
            qmfs[0] = net.sourceforge.jaad.aac.sbr.SynthesisFilterbank(if (downSampledSBR) 32 else 64)
    ...
    fun calc_sbr_tables(
        ...
        /* calculate the Master Frequency Table */k0 =
            net.sourceforge.jaad.aac.sbr.FBT.qmf_start_channel(start_freq, samplerate_mode, sample_rate)
        k2 = net.sourceforge.jaad.aac.sbr.FBT.qmf_stop_channel(stop_freq, sample_rate, k0)
        ...
        if (freq_scale == 0) {
            result += net.sourceforge.jaad.aac.sbr.FBT.master_frequency_table_fs0(this, k0, k2, alter_scale)
        } else {
            result += net.sourceforge.jaad.aac.sbr.FBT.master_frequency_table(this, k0, k2, freq_scale, alter_scale)
        }
        result += net.sourceforge.jaad.aac.sbr.FBT.derived_frequency_table(this, xover_band, k2)
    ...
    private fun sbr_single_channel_element(ld: BitStream): Int {
        ...
        sbr_envelope(ld, 0)
        sbr_noise(ld, 0)
        net.sourceforge.jaad.aac.sbr.NoiseEnvelope.dequantChannel(this, 0)
    ...
    private fun sbr_channel_pair_element(ld: BitStream): Int {
        ...
        net.sourceforge.jaad.aac.sbr.NoiseEnvelope.dequantChannel(this, 0)
        net.sourceforge.jaad.aac.sbr.NoiseEnvelope.dequantChannel(this, 1)
        if (bs_coupling) net.sourceforge.jaad.aac.sbr.NoiseEnvelope.unmap(this)
        bs_extended_data = ld.readBool()
        if (bs_extended_data) {
            var nr_bits_left: Int
            var cnt = ld.readBits(4)
    ...
    private fun sbr_grid(ld: BitStream, ch: Int): Int {
        ...
        /* TODO: this code can probably be integrated into the code above! */if (net.sourceforge.jaad.aac.sbr.TFGrid.envelope_time_border_vector(
                this,
                ch
            ).also { result = it } > 0
        ) {
        ...
        net.sourceforge.jaad.aac.sbr.TFGrid.noise_floor_time_border_vector(this, ch)
        return 0
    }
    ...
    private fun sbr_envelope(ld: BitStream, ch: Int) {
        ...
        net.sourceforge.jaad.aac.sbr.NoiseEnvelope.extract_envelope_data(this, ch)
    }
    ...
    private fun sbr_noise(ld: BitStream, ch: Int) {
        ...
        net.sourceforge.jaad.aac.sbr.NoiseEnvelope.extract_noise_floor_data(this, ch)
    }
    ...
    private fun sbr_process_channel(
        ...
        if (!dont_process) {
            ...
            net.sourceforge.jaad.aac.sbr.HFGeneration.hf_generation(this, Xsbr[ch], Xsbr[ch], ch)


            /* hf adjustment */ret =
                net.sourceforge.jaad.aac.sbr.HFAdjustment.Companion.hf_adjustment(this, Xsbr[ch], ch)
            if (ret > 0) {
                dont_process = true
            }
        }
