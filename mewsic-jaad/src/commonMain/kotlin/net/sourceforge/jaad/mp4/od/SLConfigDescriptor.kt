    ...
    override fun decode(`in`: MP4InputStream) {
        ...
        if (!predefined) {
            ...
            if (duration) {
                ...
                accessUnitDuration = `in`.readBytes(2) as Int
                compositionUnitDuration = `in`.readBytes(2) as Int
            }
            if (!useTimeStamp) {
                if (useWallClockTimeStamp) wallClockTimeStamp = `in`.readBytes(4)
                tmp = java.lang.Math.ceil((2 * timeStampLength).toDouble() / 8).toInt()
        ...
        if (ocrStream) ocrES_ID = `in`.readBytes(2) as Int
    }
}
