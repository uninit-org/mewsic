...
internal object DCT {
    ...
    private fun fft_dif(Real: FloatArray, Imag: FloatArray) {
        ...
        while (i < 16) {
            ...
            point2_real = Real[i2]
            point2_imag = Imag[i2]
            w_real = w_array_real.get(i)
            w_imag = w_array_imag.get(i)
        ...
        w_index = 0
        while (j < 8) {
            w_real = w_array_real.get(w_index)
            w_imag = w_array_imag.get(w_index)
            i = j
            point1_real = Real[i]
            point1_imag = Imag[i]
            i2 = i + 8
        ...
        w_real = w_array_real.get(4) // = sqrt(2)/2
        ...
        w_real = w_array_real.get(12) // = -sqrt(2)/2
    ...
    fun dct4_kernel(in_real: FloatArray, in_imag: FloatArray, out_real: FloatArray, out_imag: FloatArray) {
        ...
        while (i < 32) {
            ...
            x_re = in_real[i]
            x_im = in_imag[i]
            tmp = (x_re + x_im) * dct4_64_tab.get(i)
            in_real[i] = x_im * dct4_64_tab.get(i + 64) + tmp
            in_imag[i] = x_re * dct4_64_tab.get(i + 32) + tmp
            i++
        }

        /* Step 3: FFT, but with output in bit reverse order */fft_dif(
            in_real,
            in_imag
        )
        ...
        while (i < 16) {
            ...
            var x_im: Float
            var tmp: Float
            i_rev = bit_rev_tab.get(i)
            x_re = in_real[i_rev]
            x_im = in_imag[i_rev]
            tmp = (x_re + x_im) * dct4_64_tab.get(i + 3 * 32)
            out_real[i] = x_im * dct4_64_tab.get(i + 5 * 32) + tmp
            out_imag[i] = x_re * dct4_64_tab.get(i + 4 * 32) + tmp
            i++
        }
        // i = 16, i_rev = 1 = rev(16);
        out_imag[16] = (in_imag[1] - in_real[1]) * dct4_64_tab.get(16 + 3 * 32)
        out_real[16] = (in_real[1] + in_imag[1]) * dct4_64_tab.get(16 + 3 * 32)
        i = 17
        while (i < 32) {
            var x_re: Float
            var x_im: Float
