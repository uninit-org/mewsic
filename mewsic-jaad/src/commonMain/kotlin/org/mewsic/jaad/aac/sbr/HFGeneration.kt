...
internal object HFGeneration {
    ...
    fun hf_generation(
        sbr: SBR, Xlow: Array<Array<FloatArray>>,
    ...
    private fun auto_correlation(
        sbr: SBR, ac: acorr_coef, buffer: Array<Array<FloatArray>>,
        bd: Int, len: Int
    ) {
        var r01r = 0f
        var r01i = 0f
    ...
    /* FIXED POINT: bwArray = COEF */
    private fun calc_chirp_factors(sbr: SBR, ch: Int) {
        var i: Int
        i = 0
        while (i < sbr.N_Q) {
            ...
            if (sbr.bwArray.get(ch).get(i) < sbr.bwArray_prev.get(ch).get(i)) sbr.bwArray.get(ch)[i] =
                sbr.bwArray.get(ch).get(i) * 0.75f + sbr.bwArray_prev.get(ch)
                    .get(i) * 0.25f else sbr.bwArray.get(ch)[i] =
                sbr.bwArray.get(ch).get(i) * 0.90625f + sbr.bwArray_prev.get(ch).get(i) * 0.09375f
    ...
    private fun patch_construction(sbr: SBR) {
        var i: Int
        var k: Int
        var odd: Int
        var sb: Int
